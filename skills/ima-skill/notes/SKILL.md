...
### DjangoBlog 项目 Docker 部署经验（2026-04-21）

**项目路径**：`C:\Users\14031\Downloads\DjangoBlog-master`

**访问地址**：
- 前台：http://localhost:8001/（本地开发）或 http://localhost:8080/（Docker）
- 管理后台：http://localhost:8001/admin/
- 管理员：admin@example.com / admin123

**Docker 启动**：
```bash
cd C:\Users\14031\Downloads\DjangoBlog-master\deploy\docker-compose
docker-compose up -d
```

**本地开发启动**：
```bash
cd C:\Users\14031\Downloads\DjangoBlog-master
...
1. **MySQL auth_socket 问题**：默认 root@localhost 使用 auth_socket，导致 Windows 主机密码连接失败
   - 解决：创建 `mysql-init.sql`，容器启动时创建 `root@'%'` 用户使用 `mysql_native_password`
   ```sql
...
2. **nginx 静态文件路径**：同时使用 `root` 和 `alias` 导致路径重复
   - 解决：只保留 `alias`，删除 `root`

3. **nginx 端口冲突**：80 端口被占用
   - 解决：改为 `8080:80` 映射

4. **Django 5.2 BlogUserCreationForm**：`usable_password` 字段缺失导致 FieldError
   - 解决：在 `accounts/admin.py` 中添加 `usable_password` 字段

5. **文章作者必须为超级用户**：非 superuser 无法作为文章作者
   - 解决：`star_wei.is_superuser = True` 且 `is_staff = True`

6. **MySQL 健康检查**：Django 在 MySQL 就绪前启动
   - 解决：docker-compose.yml 添加 healthcheck

**Blog 功能模块**：
- accounts：用户系统（BlogUser，Django 5.2 usable_password）
- blog：文章、分类目录、标签、侧边栏、链接、网站配置
- comments：评论系统
- oauth：第三方登录
- Markdown 编辑器：Editor.md（发布内容时需用 Tab 键触发同步到隐藏 textarea）
